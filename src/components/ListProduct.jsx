...
const ListProduct = () => {
  ...
  const fetchInfo = async()=>{
    await fetch('https://dress-palette-backend-1.onrender.com/allproducts').then((res)=>
  res.json()).then((data)=>{
    setAllproducts(data)
  })
...
const remove_product = async(id)=>{
  await fetch('https://dress-palette-backend-1.onrender.com/removeproduct',{
    method:'POST',
    headers:{
              ...
              allproducts.map((product,i)=>(
                <tr className='border-b border-slate-900/20 text-gray-20 p-6 medium-14' key={i}>
                 <td className='flexStart sm:flexCenter'>
                   <img  className='rounded-lg ring-1 ring-slate-900/5 my-1' height={43} width={43} src={product.image} alt="" />
                 </td>
                 <td>
